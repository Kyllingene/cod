...
mod obj {
    ...
    impl Drawer {
        /// Create a new Drawer using stdout
        pub fn new() -> Self {
            Self{ out: Box::new(stdout()) }
        }
        ...
        /// Print an escape sequence.
        fn escape<T: std::fmt::Display>(&mut self, code: T) {
            write!(self.out, "{}[{}", 27 as char, code).unwrap();
        }

        /// Set foreground color, using 24-bit true color (not supported on all terminals).
        pub fn tc_color_fg(&mut self, r: u8, g: u8, b: u8) {
            self.escape(format!("38;2;{r};{g};{b}"));
        }

        /// Set background color, using 24-bit true color (not supported on all terminals).
        pub fn tc_color_bg(&mut self, r: u8, g: u8, b: u8) {
            self.escape(format!("48;2;{r};{g};{b}"));
        }

        /// Set foreground color, using 8-bit color.
        pub fn color_fg(&mut self, color: u8) {
            self.escape(format!("38;5;{color}m"));
        }

        /// Set background color, using 8-bit color.
        pub fn color_bg(&mut self, color: u8) {
            self.escape(format!("48;5;{color}m"));
        }

        /// Remove all color modifiers.
        pub fn decolor(&mut self) {
            self.escape("0m");
        }

        /// Clear the screen (full clear, not scroll).
        pub fn clear(&mut self) {
            write!(self.out, "{}c", 27 as char).unwrap();
        }

        /// Clear a portion of the screen.
        pub fn erase(&mut self, x1: u32, y1: u32, x2: u32, y2: u32) -> Result<(), &'static str> {
            self.rect(' ', x1, y1, x2, y2)
        }

        /// Draw a single character onto the screen.
        pub fn pixel(&mut self, c: char, x: u32, y: u32) {
            self.escape(format!("{};{}H{}", y + 1, x + 1, c));
        }

        /// Draw an orthogonal line to the screen.
        pub fn orth_line(&mut self, c: char, x1: u32, y1: u32, x2: u32, y2: u32) -> Result<(), &'static str> {
            if x1 != x2 && y1 != y2 {
                return Err("Cannot draw non-ortho lines with orth-line");
            }
        ...
        /// Draw a line onto the screen.
        pub fn line(&mut self, c: char, x1: u32, y1: u32, x2: u32, y2: u32) -> Result<(), &'static str> {
            if x1 == x2 || y1 == y2 {
                self.orth_line(c, x1, x2, y1, y2)?;
                return Ok(());
            }
        ...
        /// Draw a "texture" onto the screen.
        pub fn blit(&mut self, src: &Vec<Vec<char>>, x: u32, y: u32) {
            let mut sx = x;
            let mut sy = y;
            for row in src {
                for c in row {
        ...
        /// Draw a "texture" onto the screen.
        pub fn blit_str(&mut self, src: &String, x: u32, y: u32) {
            let split = String::from(src)
                .split('\n')
                .map(|s| s.chars())
                .map(|c| c.collect())
        ...
        /// Draw a "texture" onto the screen.
        pub fn blit_vstrs(&mut self, src: &[String], x: u32, y: u32) {
            let vec = src.iter().map(|s| s.chars()).map(|c| c.collect()).collect();
            self.blit(&vec, x, y);
        }

        /// Draw a rectangle onto the screen.
        pub fn rect(&mut self, c: char, x1: u32, y1: u32, x2: u32, y2: u32) -> Result<(), &'static str> {
            self.orth_line(c, x1, y1, x1, y2)?;
            self.orth_line(c, x1, y1, x2, y1)?;
            self.orth_line(c, x2, y2, x1, y2)?;
            self.orth_line(c, x2, y2, x2, y1)?;
        ...
        ///
        /// Putting a backslash before a character (e.g. "\\r" or "\\\") will escape it.
        pub fn ascii_box_chars<T: IntoIterator<Item = char>>(&mut self, s: T, x: u32, mut y: u32) {
            let mut escaped = false;
            let mut nx = x;
            for c in s {
                if escaped {
                ...
                match c {
                    ...
                    'r' => self.pixel(BoxDrawingChar::TopLeftCorner.into(), nx, y),
                    '-' => self.pixel(BoxDrawingChar::Horizontal.into(), nx, y),
                    'y' => self.pixel(BoxDrawingChar::TopT.into(), nx, y),
                    '7' => self.pixel(BoxDrawingChar::TopRightCorner.into(), nx, y),
                    '|' => self.pixel(BoxDrawingChar::Vertical.into(), nx, y),
                    'p' => self.pixel(BoxDrawingChar::LeftT.into(), nx, y),
                    '+' => self.pixel(BoxDrawingChar::MiddleT.into(), nx, y),
                    'd' => self.pixel(BoxDrawingChar::RightT.into(), nx, y),
                    'l' => self.pixel(BoxDrawingChar::BottomLeftCorner.into(), nx, y),
                    't' => self.pixel(BoxDrawingChar::BottomT.into(), nx, y),
                    'j' => self.pixel(BoxDrawingChar::BottomRightCorner.into(), nx, y),
        ...
        /// Draw a box using ASCII box-drawing characters.
        pub fn ascii_box(&mut self, x1: u32, y1: u32, x2: u32, y2: u32) -> Result<(), &'static str> {
            self.orth_line(BoxDrawingChar::Horizontal.into(), x1 + 1, y1, x2 - 1, y1)?;
            self.orth_line(BoxDrawingChar::Horizontal.into(), x1 + 1, y2, x2 - 1, y2)?;
            self.orth_line(BoxDrawingChar::Vertical.into(), x1, y1 + 1, x1, y2 - 1)?;
            self.orth_line(BoxDrawingChar::Vertical.into(), x2, y1 + 1, x2, y2 - 1)?;

            self.pixel(BoxDrawingChar::TopLeftCorner.into(), x1, y1);
            self.pixel(BoxDrawingChar::TopRightCorner.into(), x2, y1);
            self.pixel(BoxDrawingChar::BottomLeftCorner.into(), x1, y2);
            self.pixel(BoxDrawingChar::BottomRightCorner.into(), x2, y2);

            Ok(())
        }

        /// Draw a filled rectangle onto the screen.
        pub fn rect_fill(&mut self, c: char, x1: u32, y1: u32, x2: u32, y2: u32) -> Result<(), &'static str> {
            let mut y = y1;
            while y != y2 {
                self.orth_line(c, x1, y, x2, y)?;
                y += 1;
            }
        ...
        pub fn triangle(
            ...
            self.line(c, x1, y1, x2, y2)?;
            self.line(c, x1, y1, x3, y3)?;
            self.line(c, x3, y3, x2, y2)
        }

        /// Draw a filled triangle onto the screen.
        pub fn triangle_fill(&mut self, _c: char, _x1: u32, _y1: u32, _x2: u32, _y2: u32, _x3: u32, _y3: u32) {
            todo!();
        }

        /// Draw text onto the screen (non-wrapping, but respects linebreaks).
        pub fn text<T: IntoIterator<Item = char>>(&mut self, s: T, x: u32, mut y: u32) {
            let mut nx = x;
            for c in s {
                if c == '\n' {
                    nx = x;
